...
const rankingColumns = [
  ...
  { id: 'region', label: 'CUSTOMER NAME', minWidth: 200, align: 'left', bold: true },
...
const detailedColumns = [
  { id: 'custId', label: 'CUSTOMER ID', minWidth: 130, align: 'left', bold: true },
  { id: 'custName', label: 'CUSTOMER NAME', minWidth: 180, align: 'left' },
  { id: 'region', label: 'REGION', minWidth: 150, align: 'left' },
...
export default function CustomerSatisfactionDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (summaryRes.data) {
        setStats(prev => ({
          ...prev,
          assigned: summaryRes.data.totalFeedback || 0,
          ...
          averageScore: summaryRes.data.averageScore || 0,
          satisfactionIndex: summaryRes.data.satisfactionIndex || 0
        }));
      }
      ...
      if (mappingsRes.data) {
        ...
        const detailed = completedMappings.map(m => {
          ...
          return {
            id: m.id,
            custId: m.customerCode,
            custName: m.customerName,
            region: 'General Customer',
            question: 'Overall Satisfaction',
            rating: rating,
            score: avgScore,
            comment: m.generalComments || 'No comments',
            date: m.submitDate ? m.submitDate.split(' ')[0] : 'N/A'
          };
        });
        ...
        const rems = mappings.map(m => ({
          ...
          nextDate: '',
          status: m.status,
          mailStatus: m.status === 'Completed' ? 'Opened' : 'Sent'
        }));
        setReminders(rems);

        // Group by Customer Name for regional performance
        const custGroups = {};
        completedMappings.forEach(m => {
          const name = m.customerName || 'General';
          if (!custGroups[name]) {
            custGroups[name] = [];
          }
          custGroups[name].push(parseFloat(m.averageScore) || 0);
        });

        const mappedCustomers = Object.keys(custGroups).map((name, idx) => {
          const scores = custGroups[name];
        ...
        mappedCustomers.sort((a, b) => b.score - a.score);
        mappedCustomers.forEach((c, idx) => { c.ranking = idx + 1; });
        setCustomerAnalytics(mappedCustomers);

        // Risk Analysis
        const risks = completedMappings
          .filter(m => (parseFloat(m.averageScore) || 0) < 60)
          .map(m => {
            const avg = parseFloat(m.averageScore) || 0;
            const riskLvl = avg < 50 ? 'High' : 'Medium';
            return {
              custId: m.customerCode,
              custName: m.customerName,
              region: 'General Customer',
              score: avg,
              riskCategory: riskLvl,
              reason: riskLvl === 'High' ? 'Extremely low satisfaction average score' : 'Declining customer service indices',
              action: riskLvl === 'High' ? 'Executive Account Director review scheduled' : 'Technical account manager follow-up'
            };
          });
        setRiskAnalysis(risks);

        // Calculate highest/lowest score stats
        ...
        const allResponsesPromises = completedMappings.slice(0, 10).map(m =>
          axios.get(`/api/customer-satisfaction/customer-feedback/responses?mappingId=${m.id}`)
        );
      ...
      console.error('Error fetching customer satisfaction data:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const regionChartOptions = {
    chart: { id: 'customer-regions', toolbar: { show: false }, background: 'transparent' },
    theme: { mode: isDark ? 'dark' : 'light' },
    xaxis: {
      categories: customerAnalytics.map(c => c.region),
  ...
  const regionChartSeries = [{
    ...
    data: customerAnalytics.map(c => c.score)
  }];
    ...
    <MainCard
      ...
      title={
        <Stack direction="row" alignItems="center" spacing={1.5}>
          <IconMoodSmile size={24} color="#00796b" />
          <Box>
            <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 650, display: 'block', mb: 0.2 }}>
              Home / HRA / Customer Satisfaction
            </Typography>
            <Typography variant="h3">Customer Satisfaction Dashboard</Typography>
          </Box>
        </Stack>
      }
    >
      ...
      <Box sx={{ borderBottom: '1px solid', borderColor: 'divider', bgcolor: isDark ? 'background.default' : '#fcfcfc', p: 0.5 }}>
        ...
        >
          ...
          <Tab icon={<IconTrendingUp size={18} />} label="Customer Performance" iconPosition="start" />
              ...
              <Box sx={{
                ...
                <Box sx={{ p: 3, bgcolor: isDark ? 'rgba(255,255,255,0.01)' : 'rgba(0,0,0,0.01)', borderRadius: '12px', border: '1px solid', borderColor: 'divider', display: 'flex', flexDirection: 'column', height: '100%', flexGrow: 1 }}>
                  <Typography variant="h4" sx={{ fontWeight: 'bold', mb: 3 }}>
                    Customer Score Summary Metrics
                  </Typography>
                  <Stack spacing={2}>
                    {[
                      ...
                      { label: 'Customer Satisfaction Index', value: `${stats.satisfactionIndex?.toFixed(1)}%`, desc: 'Consolidated performance indicator.', highlight: true }
                    ].map((item, idx) => (
                      <Box key={idx} sx={{
                        display: 'flex',
                        justifyContent: 'space-between',
              ...
              <Box sx={{
                ...
                <Box sx={{ p: 3, bgcolor: isDark ? 'rgba(255,255,255,0.01)' : 'rgba(0,0,0,0.01)', borderRadius: '12px', border: '1px solid', borderColor: 'divider', display: 'flex', flexDirection: 'column', height: '100%', flexGrow: 1 }}>
                  ...
                  <BOSDataTable
                    ...
                    rows={customerAnalytics.slice(page * size, page * size + size).map(c => ({ ...c, scoreDisplay: c.score }))}
                    page={page}
                    size={size}
                    totalCount={customerAnalytics.length}
                    loading={loading}
                ...
                <Box sx={{ p: 3, bgcolor: isDark ? 'rgba(255,255,255,0.01)' : 'rgba(0,0,0,0.01)', borderRadius: '12px', border: '1px solid', borderColor: 'divider', display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center', flexGrow: 1 }}>
                  <Typography variant="h4" sx={{ fontWeight: 'bold', mb: 3 }}>
                    Customer Average Scores Visualization
                  </Typography>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 320, width: '100%' }}>
                    <Chart options={regionChartOptions} series={regionChartSeries} type="bar" height={320} style={{ width: '100%' }} />
                  </Box>
                </Box>
              </Box>
            )}
                ...
                <BOSDataTable
                  ...
                  rows={detailedResponses
                    .filter(r => {
                      ...
                      return r.custId.toLowerCase().includes(q) || r.custName.toLowerCase().includes(q);
                    })

...
import { IconTruck, IconMoodSmile, IconLock, IconArrowLeft } from '@tabler/icons-react';
...
// ─── Vendor Feedback Portal ────────────────────────────────────────────────────
// Route: /hra/satisfaction/vendor-feedback
// API:
//   GET  /api/qms/vendor-satisfaction/my-pending   → [{ mappingId, feedbackCycle, isClosed, status }]
//   GET  /api/qms/vendor-satisfaction/questions     → [{ id, satisfactionCriteria }]
//   POST /api/qms/vendor-satisfaction/submit

export default function VendorFeedbackPortal() {
  ...
  const loadPortalData = useCallback(async () => {
    setLoading(true);
    try {
      const mapRes = await axios.get('/api/qms/vendor-satisfaction/my-pending');
      ...
      if (pending) {
        ...
        const qRes = await axios.get('/api/qms/vendor-satisfaction/questions');
      ...
      console.error('Error loading vendor feedback portal', e);
  ...
  const handleSubmitSuccess = () => {
    setShowForm(false);
    setPendingMapping(null);
    dispatch(openSnackbar({ open: true, message: 'Vendor feedback submitted! Thank you.', variant: 'alert', severity: 'success' }));
    loadPortalData();
  };
  ...
  if (closedMapping) {
    return (
      <MainCard title={<Stack direction="row" alignItems="center" spacing={1.5}><IconLock size={24} style={{ color: '#9e9e9e' }} /><Typography variant="h3">Vendor Satisfaction Feedback</Typography></Stack>}>
  ...
  if (!pendingMapping) {
    return (
      <MainCard title={<Stack direction="row" alignItems="center" spacing={1.5}><IconTruck size={24} style={{ color: '#00796b' }} /><Typography variant="h3">Vendor Satisfaction Feedback</Typography></Stack>}>
        <Stack alignItems="center" spacing={2} sx={{ p: 4, textAlign: 'center' }}>
          <IconMoodSmile size={64} style={{ color: '#4caf50' }} />
          <Typography variant="h2">No Feedback Assigned</Typography>
          <Typography variant="body1" color="text.secondary" sx={{ maxWidth: '500px' }}>
            There are no active vendor satisfaction surveys awaiting your feedback. Surveys are assigned automatically based on procurement cycles.
          </Typography>
          <Button
            onClick={() => navigate(-1)}
            startIcon={<IconArrowLeft size={18} />}
  ...
  // ── Pending — Show Start Button or Form ──
  if (!showForm) {
    return (
      <>
        {/* Reminder Popup */}
        <Dialog open maxWidth="xs" fullWidth>
          <DialogTitle>
            <Stack direction="row" alignItems="center" spacing={1.5}>
              <IconTruck size={22} color="#00796b" />
              <Typography variant="h4" fontWeight={700}>Pending Feedback Reminder</Typography>
            </Stack>
          </DialogTitle>
          <DialogContent>
            <Typography variant="body1" sx={{ mb: 1 }}>
              You have a pending vendor satisfaction survey for feedback cycle:
            </Typography>
        ...
        {/* Background card */}
        <MainCard title={<Stack direction="row" alignItems="center" spacing={1.5}><IconTruck size={24} color="#00796b" /><Typography variant="h3">Vendor Satisfaction Survey</Typography></Stack>}
          secondary={<Typography variant="subtitle1" color="primary">Cycle: {pendingMapping.feedbackCycle}</Typography>}>
          <Box sx={{ p: 4, textAlign: 'center' }}>
            <Typography variant="body1" color="text.secondary">You have a pending feedback survey. Please complete it to proceed.</Typography>
  ...
  return (
    <MainCard
      title={<Stack direction="row" alignItems="center" spacing={1.5}><IconTruck size={24} color="#00796b" /><Typography variant="h3">Vendor Satisfaction Survey</Typography></Stack>}
    ...
    >
      <FeedbackSurveyForm
        ...
        questions={questions}
        submitApiUrl="/api/qms/vendor-satisfaction/submit"

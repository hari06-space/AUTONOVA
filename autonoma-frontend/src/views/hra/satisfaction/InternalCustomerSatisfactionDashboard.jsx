...
export default function InternalCustomerSatisfactionDashboard() {
  ...
  const [internalAnalytics, setInternalAnalytics] = useState([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // 1. Dashboard summary
      const summaryRes = await axios.get('/api/qms/internal-customer-satisfaction/dashboard-summary', { params });
      if (summaryRes.data) {
        setStats(prev => ({
          ...prev,
          ...
          averageScore: summaryRes.data.averageSatisfactionScore || 0,
          satisfactionIndex: summaryRes.data.averageSatisfactionScore || 0
        }));
      }

      // 2. Mappings list
      const mappingsRes = await axios.get('/api/qms/internal-customer-satisfaction/mappings', { params });
      if (mappingsRes.data) {
        ...
        // Group by Department for rankings
        const deptGroups = {};
        completedMappings.forEach(m => {
          ...
          deptGroups[dept].push(parseFloat(m.averageScore) || 0);
        });

        const mappedDepts = Object.keys(deptGroups).map((dept, idx) => {
          const scores = deptGroups[dept];
        ...
        mappedDepts.sort((a, b) => b.score - a.score);
        mappedDepts.forEach((d, idx) => { d.ranking = idx + 1; });
        setInternalAnalytics(mappedDepts);
        ...
        const risks = mappings
          ...
          .map(m => ({
            ...
            reason: m.riskLevel === 'High' ? 'Low alignment scores across multiple cycles' : 'Slower response speeds reported',
            action: m.riskLevel === 'High' ? 'Internal alignment workshop scheduled' : 'Standard service level review'
          }));
        ...
        const allResponsesPromises = completedMappings.slice(0, 10).map(m =>
          axios.get(`/api/qms/internal-customer-satisfaction/responses?mappingId=${m.id}`)
        );
      ...
      console.error('Error fetching internal customer satisfaction data:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const deptChartOptions = {
    chart: { id: 'internal-depts', toolbar: { show: false }, background: 'transparent' },
    theme: { mode: isDark ? 'dark' : 'light' },
    xaxis: {
      categories: internalAnalytics.map(d => d.department),
  ...
  const deptChartSeries = [{
    ...
    data: internalAnalytics.map(d => d.score)
  }];
        ...
        <Stack direction="row" alignItems="center" spacing={1.5}>
          ...
          <Box>
            <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 650, display: 'block', mb: 0.2 }}>
              Home / HRA / Internal Customer Satisfaction
            </Typography>
            <Typography variant="h3">Internal Customer Satisfaction Dashboard</Typography>
          </Box>
        </Stack>
      }
    >
      ...
      <Box sx={{ borderBottom: '1px solid', borderColor: 'divider', bgcolor: isDark ? 'background.default' : '#fcfcfc', p: 0.5 }}>
        ...
        >
          ...
          <Tab icon={<IconTrendingUp size={18} />} label="Department Performance" iconPosition="start" />
              ...
              <Box sx={{
                ...
                <Box sx={{ p: 3, bgcolor: isDark ? 'rgba(255,255,255,0.01)' : 'rgba(0,0,0,0.01)', borderRadius: '12px', border: '1px solid', borderColor: 'divider', display: 'flex', flexDirection: 'column', height: '100%', flexGrow: 1 }}>
                  <Typography variant="h4" sx={{ fontWeight: 'bold', mb: 3 }}>
                    Internal Customer Score Summary Metrics
                  </Typography>
                  <Stack spacing={2}>
                    {[
                      ...
                      { label: 'Internal Customer Satisfaction Index', value: `${stats.satisfactionIndex?.toFixed(1)}%`, desc: 'Consolidated performance indicator.', highlight: true }
                    ].map((item, idx) => (
                      <Box key={idx} sx={{
                        display: 'flex',
                        justifyContent: 'space-between',
              ...
              <Box sx={{
                ...
                <Box sx={{ p: 3, bgcolor: isDark ? 'rgba(255,255,255,0.01)' : 'rgba(0,0,0,0.01)', borderRadius: '12px', border: '1px solid', borderColor: 'divider', display: 'flex', flexDirection: 'column', height: '100%', flexGrow: 1 }}>
                  ...
                  <BOSDataTable
                    ...
                    rows={internalAnalytics.slice(page * size, page * size + size).map(d => ({ ...d, scoreDisplay: d.score }))}
                    page={page}
                    size={size}
                    totalCount={internalAnalytics.length}
                    loading={loading}
                ...
                <Box sx={{ p: 3, bgcolor: isDark ? 'rgba(255,255,255,0.01)' : 'rgba(0,0,0,0.01)', borderRadius: '12px', border: '1px solid', borderColor: 'divider', display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center', flexGrow: 1 }}>
                  <Typography variant="h4" sx={{ fontWeight: 'bold', mb: 3 }}>
                    Department Average Scores Visualization
                  </Typography>

...
export default function LeaveMaster() {
  ...
  const leaveMasterColumns = useMemo(() => {
    ...
    return [
      ...
      { id: 'empCode', label: 'Employee Code', bold: true, minWidth: 140 },
  ...
  return (
    ...
    >
      ...
      <BOSFormDialog
        ...
        maxWidth={!formData.id ? 'md' : 'lg'}
        fullWidth
      >
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: !formData.id ? 2 : 3, pt: 1, pb: 1, width: '100%' }}>
          <Box sx={{
            display: 'grid',
            gridTemplateColumns: formData.id
              ? { xs: '1fr', md: '3.5fr 5fr 3.5fr' } // Edit Mode: 3 columns (Left: Profile, Center: Selection/Balances, Right: History)
              : { xs: '1fr', md: '3.4fr 6.6fr' }, // Add Mode: 2 compact columns in medium dialog
            gap: !formData.id ? 2.5 : 3,
            width: '100%',
            alignItems: 'stretch'
          }}>
            {/* Left Panel: Employee Profile Card (Matching Leave Apply 100%) */}
            <Box sx={{
              border: '1.5px solid',
              borderColor: 'divider',
              borderRadius: '16px',
              bgcolor: isDark ? 'background.default' : 'grey.50',
              p: !formData.id ? 2.5 : 3.5,
              display: 'flex',
              flexDirection: 'column',
              justifyContent: 'center',
              boxShadow: '0 4px 12px rgba(0,0,0,0.05)',
              boxSizing: 'border-box',
              minHeight: !formData.id ? '310px' : '380px',
              position: 'sticky',
              top: '24px',
              alignSelf: 'start',
              zIndex: 1
              ...
              {!selectedEmployee || !employeeProfile ? (
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1.5, py: 2, color: 'text.secondary', textAlign: 'center', height: '100%' }}>
                  <Avatar sx={{ width: 44, height: 44, bgcolor: isDark ? 'rgba(255,255,255,0.05)' : 'grey.100', color: 'text.secondary' }}>
                    <IconUser size={22} />
                  </Avatar>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>Select an employee to view details</Typography>
                </Box>
              ) : (
                <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: !formData.id ? 1.5 : 2, textAlign: 'center' }}>
                  <Avatar
                    ...
                    sx={{
                      width: !formData.id ? 72 : 80,
                      height: !formData.id ? 72 : 80,
                      border: '3px solid',
                      borderColor: 'primary.main',
                      boxShadow: 2
                    }}
                  >
                    {employeeProfile.employeeName?.charAt(0) || <IconUser size={36} />}
                  </Avatar>
                  <Box>
                    ...
                    <Typography variant="subtitle2" color="primary.main" sx={{ fontWeight: 700, mb: 1 }}>
                      Employee ID: {employeeProfile.empCode || employeeProfile.oldEmpCode || '—'}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 1.5, textAlign: 'left' }}>
                    <Box>
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 600, mb: 1.5 }}>{getDepartmentString(employeeProfile)}</Typography>
                    </Box>
                    <Box>
                      <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700, display: 'block' }}>Designation</Typography>
                      <Typography variant="body2" sx={{ fontWeight: 600 }}>{getDesignationString(employeeProfile)}</Typography>
                    </Box>
                  </Box>
                </Box>
              )}
            ...
            {/* Center/Right Panel: Selection & Current Leave Balances */}
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: !formData.id ? 2 : 3 }}>
              {/* Employee Selection */}
              <BOSFormSection icon={<IconUser size={!formData.id ? 20 : 22} color={theme.palette.primary.main} />} title="Employee Selection">
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: !formData.id ? 1.5 : 2.5 }}>
                  <BOSAutocomplete
                    name="employeeId"
                    label="Employee Name *"
                    ...
                    fullWidth
                  />
                  <Box sx={{ width: { xs: '100%', sm: !formData.id ? '220px' : '280px' } }}>
                    <BOSStatusField
              ...
              {/* Current Leave Balances Panel (Matching Leave Apply List-Item Panel Design) */}
              <BOSFormSection icon={<IconClock size={!formData.id ? 20 : 22} color={theme.palette.primary.main} />} title="Current Leave Balances">
                <Box sx={{
                  display: 'flex',
                  flexDirection: 'column',
                  gap: !formData.id ? 1 : 1.5,
                  maxHeight: !formData.id ? '290px' : '380px',
                  overflowY: 'auto',
                  pr: 0.5,
                  width: '100%'
                }}>
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          borderRadius: !formData.id ? '8px' : '10px',
                          px: !formData.id ? 2 : 2.5,
                          py: !formData.id ? 0.75 : 1.2,
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      >
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: !formData.id ? 1.2 : 1.8 }}>
                          <Chip
                            label={item.code}
                            size="small"
                            sx={{
                              fontWeight: '800',
                              fontSize: !formData.id ? '0.7rem' : '0.75rem',
                              borderRadius: '6px',
                              bgcolor: colors.badge,
                              color: '#ffffff',
                              height: !formData.id ? '22px' : '24px',
                              minWidth: !formData.id ? '34px' : '38px'
                            }}
                          />
                          <Typography variant="body2" sx={{ fontWeight: 800, color: colors.text, fontSize: !formData.id ? '0.8rem' : '0.85rem', letterSpacing: 0.2 }}>
                            {item.name} ({item.code})
                          </Typography>
                        </Box>

                        <Box sx={{ width: !formData.id ? '110px' : '140px' }}>
                          <BOSTextField
                            name={item.key}
                            type="number"
                            ...
                            sx={{
                              ...
                              '& .MuiInputBase-input': {
                                fontWeight: 900,
                                fontSize: !formData.id ? '0.875rem' : '1rem',
                                py: !formData.id ? '4px' : '8.5px',
                                px: !formData.id ? '8px' : '14px',
                                textAlign: 'right',
                                color: colors.text
                              },
                              '& .MuiOutlinedInput-root': {
                                bgcolor: isDark ? 'background.paper' : '#ffffff',
                                borderRadius: !formData.id ? '6px' : '8px'
                              }
                            }}
                          />
                        </Box>
            ...
            {/* Right Panel: Leave Allocation History (Edit Mode only) */}
            {formData.id && (
              <Box sx={{
                border: '1px solid',
                borderColor: 'divider',
                    ...
                    {sortedEmployeeCreditTransactions.map((tx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {tx.remarks && (
                            <Box sx={{ mt: 0.5 }}>
                              <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, display: 'block' }}>Remarks</Typography>
                              <Typography variant="body2" sx={{ fontStyle: 'italic', wordBreak: 'break-word' }}>{tx.remarks}</Typography>
                            </Box>
                          )}
                        </Box>
                      );

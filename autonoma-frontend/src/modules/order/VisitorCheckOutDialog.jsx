...
export default function VisitorCheckOutDialog({ open, onClose, onConfirm, visitor }) {
  ...
  useEffect(() => {
    ...
    hours = hours % 12 || 12;
    setOutTime(`${String(hours).padStart(2, '0')}:${mins} ${ampm}`);
    ...
    if (currentPassImg) {
      ...
      setPhotoMessage('Auto-fetched photo from Check-In.');
      return;
    }

    if (!mobile) {
      setIsPhotoMandatory(true);
      setPhotoMessage('New visitor (no mobile number). Photo capture is MANDATORY.');
      startWebcam(facingMode, currentDeviceId);
      return;
    }

    // Query backend for 6-month threshold & latest photo
  ...
  const capturePhoto = async () => {
    const video = videoRef.current || document.getElementById('v-webcam-video-out');
    if (!video) return;
  ...
  const handleConfirm = () => {
    ...
    const now = new Date();
    const formattedOutTime = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
    onConfirm({
      image: capturedImage,
      ...
      <DialogContent sx={{ p: { xs: 1.5, sm: 2.5 }, display: 'flex', flexDirection: 'column', alignItems: 'center', overflowY: 'auto' }}>
        ...
        {loadingPhoto ? (
          ...
          <Box
            sx={{
              ...
              position: 'relative',
              border: '2px solid',
              borderColor: capturedImage ? '#3b82f6' : '#64748b',
        ...
        <Stack spacing={0.5} sx={{ width: '100%' }}>
          <Stack direction="row" alignItems="center" spacing={1} sx={{ color: 'text.secondary', px: 0.5 }}>
            <IconClock size={15} />
            <Typography variant="caption" sx={{ fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, fontSize: '0.7rem' }}>OUT Time</Typography>
          </Stack>
          <Box
            sx={{
          ...
          >
            <Stack direction="row" alignItems="center" spacing={1.2} sx={{ color: '#0f172a' }}>
              ...
              <Typography variant="h4" sx={{ fontWeight: 800, fontSize: '1.15rem' }}>{outTime}</Typography>
            </Stack>
          </Box>
        </Stack>

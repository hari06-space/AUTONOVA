...
const columns = [
  ...
  { id: 'referenceCode', label: 'CUST CODE', minWidth: 120 },
...
export default function CustomerMasterList() {
  ...
  useEffect(() => {
    const config = [{ id: 'customerName', label: 'Customer Name', type: 'text', placeholder: 'Search by Name...', isStarred: true },
  ...
  const fetchCustomers = useCallback(async () => {
    setLoading(true);
      try {
        const [response, contactsRes] = await Promise.all([
          axios.get('/api/master/vendors'),
          axios.get('/api/sm/contacts')
        ]);
        const contacts = contactsRes.data || [];
        const filteredCustomers = response.data.filter(v => v.isCustomer);
        const mappedData = filteredCustomers.map(v => {
        ...
        negotiateCustomer: v.negotiateRequired,

...
import { IconDeviceFloppy, IconArrowLeft, IconTrash, IconEraser, IconPaperclip, IconUser, IconMapPin, IconBusinessplan, IconTruckDelivery, IconCurrentLocation, IconExternalLink, IconSearch, IconX } from '@tabler/icons-react';
...
export default function SupplierMaster() {
  ...
  useEffect(() => {
    ...
    axios.get('/api/master/vendors?type=supplier').then(res => setAllVendors(res.data)).catch(console.error);
  ...
  const handleVendorSelect = (val) => {
    const match = allVendors.filter(v => (v.isSupplier || v.isSubcon)).find(v => v.vendorName === val || v.gstin === val);
    if (match) {
      navigate(`/sm/vendors/edit/${match.vendorId || match.id}`, { replace: true });
    } else {
  ...
  const handleGstinSelect = (val) => {
    const match = allVendors.filter(v => (v.isSupplier || v.isSubcon)).find(v => v.gstin === val || v.vendorName === val);
    if (match) {
      navigate(`/sm/vendors/edit/${match.vendorId || match.id}`, { replace: true });
    } else {
  ...
  const handleSave = async () => {
    ...
    try {
      if (vendorId) {
        ...
        dispatch(openSnackbar({ open: true, message: 'Vendor Updated Successfully', variant: 'alert', alert: { color: 'success' } }));
      } else {
        await axios.post('/api/master/vendors', form);
        dispatch(openSnackbar({ open: true, message: 'Vendor Created Successfully', variant: 'alert', alert: { color: 'success' } }));
        navigate('/sm/vendors');
      }
    } catch (error) {
      dispatch(openSnackbar({ open: true, message: error?.response?.data?.message || 'Error saving vendor', variant: 'alert', alert: { color: 'error' } }));
    } finally {
      setLoading(false);
    }
  };
    ...
    <MainCard
      title={
        <Stack direction="row" alignItems="center" spacing={2}>
          <Typography variant="h3">{vendorId ? 'Edit Vendor' : 'New Vendor'}</Typography>
        </Stack>
      }
      secondary={
        <Stack direction="row" spacing={1}>
          ...
          <Button variant="contained" color="secondary" onClick={() => navigate('/sm/vendors')} startIcon={<IconArrowLeft size={18} />} sx={btnCancel}>
            Back
          </Button>
          {!vendorId && (
    ...
    >
      <Stack spacing={3}>
        <Box>
          ...
          <BOSFormSection icon={<IconUser size={20} color={theme.palette.primary.main} />} title="Identity & Profile">
            <Box sx={{ width: '100%', px: 1 }}>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2.5, width: '100%', alignItems: 'flex-start' }}>
                <Box sx={{ flex: '1 1 32%' }}><BOSAutocomplete freeSolo fullWidth options={Array.from(new Set(allVendors.filter(v => (v.isSupplier || v.isSubcon)).map(v => v.vendorName).filter(Boolean)))} value={form.vendorName || null} onInputChange={(e, val) => { if (e && e.type === 'change') setForm(p => ({ ...p, vendorName: val })); }} onChange={(val) => handleVendorSelect(val)} label="Customer Name *" error={!!errors.vendorName} helperText={errors.vendorName} /></Box>
          ...
          <BOSFormSection icon={<IconMapPin size={20} color={theme.palette.primary.main} />} title="Contact & Location Details">
            <Box sx={{ width: '100%', px: 1 }}>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2.5, width: '100%', alignItems: 'flex-start', mb: 2.5 }}>
                <Box sx={fbx}><BOSTextField fullWidth name="mobileNo" label="Mobile No" value={form.mobileNo} onChange={h} country={form.country} /></Box>
                <Box sx={fbx}><BOSTextField fullWidth name="mailId" label="Mail ID" value={form.mailId} onChange={h} type="email" /></Box>
                <Box sx={fbx}><BOSTextField fullWidth name="officeNo" label="Office No" value={form.officeNo} onChange={h} country={form.country} /></Box>
          ...
          <BOSFormSection icon={<IconBusinessplan size={20} color={theme.palette.primary.main} />} title="Statutory & Registration">
            <Box sx={{ width: '100%', px: 1 }}>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2.5, width: '100%', alignItems: 'flex-start' }}>
                <Box sx={fbx}><BOSAutocomplete freeSolo fullWidth options={Array.from(new Set(allVendors.filter(v => (v.isSupplier || v.isSubcon)).map(v => v.gstin).filter(Boolean)))} value={form.gstin || null} onInputChange={(e, val) => { if (e && e.type === 'change') setForm(p => ({ ...p, gstin: val })); }} onChange={(val) => handleGstinSelect(val)} label="GSTIN *" error={!!errors.gstin} helperText={errors.gstin} /></Box>
          ...
          <BOSFormSection icon={<IconBusinessplan size={20} color={theme.palette.primary.main} />} title="Financial Mapping">
            <Box sx={{ width: '100%', px: 1 }}>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2.5, width: '100%', alignItems: 'flex-start' }}>
                ...
                <Box sx={fbx}><BOSAutocomplete fullWidth value={taxLedgers.find(v => v.id === form.tdsLedgerId)?.ledgerName || null} onChange={(val) => setForm(p => ({ ...p, tdsLedgerId: taxLedgers.find(v => v.ledgerName === val)?.id || null }))} options={taxLedgers.filter(v => (v.category || '').toUpperCase() === 'TDS').map(v => v.ledgerName)} label="TDS Ledger" /></Box>
              </Box>
            </Box>
          </BOSFormSection>
          ...
          <BOSFormSection icon={<IconTruckDelivery size={20} color={theme.palette.primary.main} />} title="Terms & Logistics">
            <Box sx={{ width: '100%', px: 1 }}>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2.5, width: '100%', alignItems: 'flex-start' }}>
                <Box sx={fbx}><BOSAutocomplete fullWidth value={form.currencyCode || null} onChange={handleAC('currencyCode')} options={currencies.map(c => c.currencyName || c.currencyCode)} label="Currency" /></Box>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 4, width: '100%', alignItems: 'center', mt: 3, mb: 1, px: 2 }}>
                <BOSToggleSwitch name="primeVendor" value={form.primeVendor} onChange={h} checkedLabel="" uncheckedLabel="" label="Prime Vendor" />
      ...
      {attachmentOpen && (
        <VendorAttachmentDialog
          ...
          vendorId={vendorId}
          pageCode="VENDOR_MASTER"
          docTypes={['Logo', 'GST Certificate', 'PAN Card', 'Certificate of Incorporation', 'Address Proof', 'Cancelled Cheque', 'Bank Verification Letter', 'Supplier Agreement', 'NDA', 'SLA', 'Trade License', 'Business License', 'Insurance Certificate', 'Product Catalogue', 'Rate Card', 'Company Profile', 'ISO Certificate', 'OEM Authorization', 'Other Documents']}
        />
      )}

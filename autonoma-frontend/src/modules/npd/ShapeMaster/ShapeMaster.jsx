...
import AddShapeMasterDialog from './AddShapeMasterDialog';
...
const columns = [
  ...
  { id: 'shapeName', label: 'Shape Name', minWidth: 200 },
  { id: 'description', label: 'Description', minWidth: 250 },
  { id: 'dimensionType', label: 'Dimension Type', minWidth: 150 },
...
export default function ShapeMaster() {
  const dispatch = useDispatch();
  const globalQuery = useSelector((state) => state.search.query);
  const globalFilters = useSelector((state) => state.search.filters);
  const perms = usePagePermissions(PAGE_CODES.NPD_SHAPE_MASTER);
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      ...
      const matchesSearch = !globalQuery ||
        ...
        (row.shapeName && row.shapeName.toLowerCase().includes(globalQuery.toLowerCase()));
  ...
  return (
    <MainCard fullWidth
      ...
      secondary={
        <BOSTableToolbar
          onRefresh={fetchShapes}
          onNew={handleOpenAdd}
          newTooltip={shortcutTooltip('Create New Shape', 'Ctrl + N')}
          hasWritePermission={perms.write}
          exportData={filteredRows}
          exportFilename="Shape_Master"
    ...
    >
      ...
      <AddShapeMasterDialog open={dialogOpen} handleClose={handleCloseDialog} initialData={selectedRow} readOnly={isReadOnly} />

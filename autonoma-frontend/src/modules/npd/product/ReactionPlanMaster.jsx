...
import AddReactionPlanDialog from './AddReactionPlanDialog';
...
const columns = [
  ...
  { 
    ...
    label: 'Reaction Plan', 
    minWidth: 250,
    render: (row) => row.reactionPlan ? row.reactionPlan.replace(/<[^>]*>/g, '').trim() : ''
  },
...
export default function ReactionPlanMaster() {
  const dispatch = useDispatch();
  const globalQuery = useSelector((state) => state.search.query);
  const globalFilters = useSelector((state) => state.search.filters);
  const perms = usePagePermissions(PAGE_CODES.NPD_REACTION_PLAN);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get(API_PATHS.NPD.REACTION_PLAN);
      setRows(response.data);
    } catch (error) {
      console.error('Failed to fetch reaction plans:', error);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await axios.delete(`${API_PATHS.NPD.REACTION_PLAN}/${deleteTargetRow.id}`);
      dispatch(openSnackbar({ open: true, message: 'Reaction Plan deleted successfully!', variant: 'alert', alert: { variant: 'filled' }, severity: 'success', close: false }));
      fetchData();
    } catch (error) {
  ...
  const filteredRows = useMemo(() => {
    return processedRows.filter((row) => {
      ...
      const matchesSearch = !globalQuery ||
        ...
        (row.reactionPlan && row.reactionPlan.toLowerCase().includes(globalQuery.toLowerCase()));
  ...
  return (
    <MainCard
      ...
      icon={IconFlame}
      title={"Reaction Plan Master"}
      secondary={
        <BOSTableToolbar
          id="npd-reaction-plan-toolbar"
          onRefresh={fetchData}
          onNew={handleOpenAdd}
          newTooltip={shortcutTooltip('Create New Reaction Plan', 'Ctrl + N')}
          hasWritePermission={perms.write}
          exportData={filteredRows}
          exportFilename="Reaction_Plan_Master"
    ...
    >
      ...
      <AddReactionPlanDialog open={dialogOpen} handleClose={handleCloseDialog} initialData={selectedRow} readOnly={isReadOnly} />

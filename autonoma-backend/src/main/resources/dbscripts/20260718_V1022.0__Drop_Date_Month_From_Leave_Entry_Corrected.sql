-- V1022.0: Correctly drop DATE and MONTH columns from HR_LEAVE_ENTRY by dropping dependent index IX_LeaveEntry_Date first.

-- 1. Drop index IX_LeaveEntry_Date if it exists
IF EXISTS (
    SELECT 1 FROM sys.indexes 
    WHERE name = 'IX_LeaveEntry_Date' 
      AND object_id = OBJECT_ID('HR_LEAVE_ENTRY')
)
BEGIN
    DROP INDEX IX_LeaveEntry_Date ON HR_LEAVE_ENTRY;
END

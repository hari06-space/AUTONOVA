...
/**
 * PURCHASE_REQUEST strategy: loads approved PR items and pre-populates PO.
 * Supplier is still selected manually by buyer.
 */
@Component
@RequiredArgsConstructor
public class PurchaseRequestPoStrategy implements PurchaseOrderSourceStrategy {

    private final JdbcTemplate jdbcTemplate;

    @Override
    public boolean supports(PoSourceType sourceType) {
        return PoSourceType.PURCHASE_REQUEST == sourceType;
    }

    @Override
    public PurchaseOrderHeadDTO resolve(Long sourceDocId, Long divisionId) {
        // Fetch PR Header
        String headerSql = "SELECT h.PR_NO, h.REMARKS FROM PP_PURCHASE_REQUEST_HEAD h WITH(NOLOCK) WHERE h.ID = ?";
        List<PurchaseOrderHeadDTO> headers = jdbcTemplate.query(headerSql, (rs, n) -> {
            PurchaseOrderHeadDTO dto = new PurchaseOrderHeadDTO();
            dto.setSourceType(PoSourceType.PURCHASE_REQUEST.name());
            dto.setSourceDocumentNo(rs.getString("PR_NO"));
        ...
        if (headers.isEmpty())
            throw new BusinessException("Purchase Request not found: " + sourceDocId);
        PurchaseOrderHeadDTO dto = headers.get(0);

        // Fetch PR Items
        String itemSql = "SELECT t.ID, t.ITEM_ID, p.ITEM_NAME, p.ITEM_CODE, p.HSN_CODE, COALESCE(t.UOM, p.UOM) AS UOM, t.REQ_QTY, t.REMARKS, " +
                "COALESCE(hsn.CGST_PER, 0) AS CGST_PERCENT, " +
                "COALESCE(hsn.SGST_PER, 0) AS SGST_PERCENT, " +
                "COALESCE(hsn.IGST_PER, 0) AS IGST_PERCENT " +
                "FROM PP_PURCHASE_REQUEST_TRANS t WITH(NOLOCK) " +
                ...
                "WHERE t.PR_REF_ID = ? AND t.STATUS_ID = (SELECT ID FROM AD_STATUS_MASTER WHERE UPPER(TRIM(NAME)) = 'APPROVED')";
        ...
        src.setSourceType(PoSourceType.PURCHASE_REQUEST.name());

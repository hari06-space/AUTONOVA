...
@Component
public class DeliveryChallanSourceStrategy implements GateEntrySourceStrategy {
    ...
    public void validateSource(GateEntrySourceDTO sourceDto, GateEntryHeadDTO headDto) {
        if (sourceDto.getSourceDocumentNo() == null || sourceDto.getSourceDocumentNo().isEmpty()) {
            throw new IllegalArgumentException("Challan Number is mandatory for Delivery Challan source.");
        }
        // Validation: Prevent duplicate challan for same supplier
    }
    ...
    public void populateItemsFromSource(GateEntrySourceDTO sourceDto, List<GateEntryTransDTO> targetList) {
        // Not applicable for Challan unless fetched from an external ASN
    }
}

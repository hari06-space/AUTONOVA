...
$body = @{
    email = "EMP-001"
    password = "EMP-001"
    tenantId = "ERPDb_NUTECH"
    divisionId = $null
} | ConvertTo-Json

try {
    Write-Host "Logging in as EMP-001 with tenant ERPDb_NUTECH..."
    ...
    Write-Host "Calling verify endpoint with tenant ERPDb_NUTECH..."
